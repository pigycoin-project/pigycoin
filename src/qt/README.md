This directory contains the PigycoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Pigycoin UI.
...
ClientModel is used by the main application `pigycoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/pigycoin/pigycoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a pigycoin:... link from a browser).
...
* UI elements like PigycoinAmountField, which inherit from QWidget.
* `pigycoinstrings.cpp`: automatically generated
* `pigycoinunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/pigycoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "pigycoin-qt" under "Run", which is where you can also add command line arguments)
