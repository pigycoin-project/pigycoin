...
const char * const PIGYCOIN_CONF_FILENAME = "pigycoin.conf";
const char * const PIGYCOIN_PID_FILENAME = "pigycoin.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pigycoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pigycoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pigycoin
    // Mac: ~/Library/Application Support/Pigycoin
    // Unix: ~/.pigycoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PIGYCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", PIGYCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Pigycoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Pigycoin Core") == std::string::npos) {
