Pigycoin Core version 1.00 is now available from:
  ...
  <https://download.pigycoin.org/pigycoin-1.00/>
...
- The GUI suffers from visual glitches in the new MacOS dark mode. This has to do with our Qt theme handling and is not a new problem in 0.17.0, but is expected to be resolved in 1.00.
...
When pigycoin is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `pigycoin.conf` file, not inside included files or from
  ...
  as pigycoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the pigycoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
The default minimum transaction fee `-mintxfee` has been lowered to 0.0001 PIGY/kB after relaxing the minimum relay and dust relay fee rates in prior releases. 
...
prefixing the option with the network, such as:
    ...
    main.uacomment=pigycoin
    test.uacomment=pigycoin-testnet
...
be used in V0.17 by starting pigycoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Pigycoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/pigycoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Pigycoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://pigycointalk.org/?topic=279249) transactions.

For backend RPC convenience, the Pigycoin devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed Pigycoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Pigycoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in PIGY. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When pigycoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If pigycoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart pigycoind with
...
- When running pigycoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If pigycoind
...
- #11962 `0a01843` add seed.pigycoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI pigycoin:// (krab)
...
- #13528 `b877c39` Move PigycoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libpigycoin-util to libpigycoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for pigycoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling pigycoin-qt (TheCharlatan)
...
- #14698 `ec71f06` Add pigycoin-tx.exe into Windows installer (ken2812221)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use PigycoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to pigycoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use PigycoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use PigycoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use PigycoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for PigycoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_pigycoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.pigycoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_pigycoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in pigycoind systemd service file (Flowdalic)
- #12652 `55f490a` pigycoin-cli: Provide a better error message when pigycoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to pigycoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for pigycoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` pigycoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix pigycoin-cli --version (Empact)
...
- #12877 `2b54155` Use pigycoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update pigycoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust pigycoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed pigycoin-cli -help output for help2man (hebasto)
...
- [The Pigycoin Core Developers](https://github.com/pigycoin/pigycoin/tree/master/doc/release-notes)
- Adrian Gallagher
- aunyks
- coblee
- cryptonexii
