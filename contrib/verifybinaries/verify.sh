...
###   pigycoincore.org and pigycoin.org and compares them.
...
WORKINGDIR="/tmp/pigycoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://pigycoincore.org"
HOST2="https://pigycoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="pigycoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pigycoin-',
...
if [ $? -ne 0 ]; then
   echo "pigycoin.org failed to provide signature file, but pigycoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "pigycoin.org and pigycoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pigycoin Core binary release signing key installed?"
   fi
